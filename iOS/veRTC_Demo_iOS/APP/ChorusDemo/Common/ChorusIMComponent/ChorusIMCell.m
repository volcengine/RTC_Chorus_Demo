...
#import "ChorusIMCell.h"

@interface ChorusIMCell ()
...
- (void)setModel:(ChorusIMModel *)model {
    _model = model;
    ...
    if (NOEmptyStr(model.message)) {
        ...
        NSString *unitStr = model.isJoin ? veString(@"加入了房间") : veString(@"退出了房间");
